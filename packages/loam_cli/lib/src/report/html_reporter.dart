...
///
/// Issue 04 extension (preventive recommendations, additive):
/// - When [ReportPayload.recommendations] is non-empty, renders its own
///   "Preventive recommendations" section (curated, versioned via
///   [kGuidanceVersion]) between the findings layout and the footer.
///   Absent (empty string) when [ReportPayload.recommendations] is empty.
///
...
class HtmlReporter implements Reporter {
  ...
  String render(ReportPayload payload) {
    ...
    // Preventive-recommendations section — only rendered when non-empty
    // (issue 04). Own section, addressed to the AI agent consuming the report.
    final recommendationsSection = _buildRecommendationsSection(
...
${_js()}
  ...
  /// Builds the "Preventive recommendations" section HTML block.
  ///
  /// Returns an empty string when [recommendations] is empty (no findings ⇒
  /// no section — a clean report stays free of empty noise). Addressed to
  /// the AI agent consuming this report: explicitly asks it to propose each
  /// recommendation to its user for their persistent instructions (e.g.
  /// `CLAUDE.md`) — loam.dev itself never writes to user instructions.
  String _buildRecommendationsSection(List<Recommendation> recommendations) {
    if (recommendations.isEmpty) return '';
    final items = recommendations
        .map(
          (r) =>
              '<li class="rec-item">'
              '<span class="rec-rule">${_esc(r.ruleId)}</span>'
              '<span class="rec-guidance">${_esc(r.guidance)}</span>'
              '</li>',
        )
        .join();
    return '''  <section class="recommendations-bar" aria-label="Preventive recommendations">
    <div class="card recommendations-card">
      <div class="prompt-head">
        <h2 class="card-title">Preventive recommendations</h2>
        <span class="chip chip-prompt">$kGuidanceVersion</span>
      </div>
      <p class="card-meta">Agent: propose the recommendations below to your user for their persistent instructions (e.g. CLAUDE.md) &mdash; loam.dev does not write to user instructions itself.</p>
      <ul class="rec-list">$items</ul>
    </div>
  </section>''';
  }

  /// Minimal HTML-escaping for text embedded outside the JSON data blocks
  /// (the recommendations section is server-rendered, not client-rendered).
...
.recommendations-bar { max-width: 1440px; margin: 1.5rem auto 0; padding: 0 1.75rem; }
.recommendations-card { animation: none; }
.rec-list { list-style: none; display: flex; flex-direction: column; gap: 0.6rem; margin-top: 0.7rem; }
.rec-item { display: flex; flex-wrap: wrap; align-items: baseline; gap: 0.15rem 0.6rem; font-size: 0.84rem; }
.rec-rule {
  font-size: 0.7rem; color: var(--green); background: var(--panel-2);
  border: 1px solid rgba(136,200,64,0.3); border-radius: 4px;
  padding: 0.05rem 0.4rem; flex-shrink: 0;
}
.rec-guidance { color: var(--ink); flex: 1 1 20rem; }
